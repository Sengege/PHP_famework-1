...
$(function(){(
	function(){
		...
		$("a em",a).live("click",function(){
			...
			delTips(b,d)
			$.post('../classes/routers/skills_update_router.php', {delete_skill_id : d})
		});
		...
		setTips=function(c,d){
			if(hasTips(c)){
				...
				alert("Can't have more than "+G_tocard_maxTips+" tags！");
				return false
			}
			var b=d?'value="'+d+'"':"";
			//a.append($("<a "+b+' title="'+c+'" href="javascript:void(0);" ><span>'+c+"</span><em></em></a>"));
			$.post('../classes/routers/skills_update_router.php', {add_skill_id : d, add_skill_name : c}, function (data) {
				if(data=="true"){
					a.append($("<a "+b+' title="'+c+'" href="javascript:void(0);" ><span>'+c+"</span><em></em></a>"));
					searchAjax(c,d,true);
					return true
